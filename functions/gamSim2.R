#### This function allow to simulate a multivariate with one center

gamSim2 <- function(eg = 1, n = 400, dist = "normal", scale = 2, verbose = TRUE) 
{
  if (eg == 1 || eg == 7) {
    if (eg == 1) {
      if (verbose) 
    ...
    #                                                                               (z - 0.8)^2/sz^2))
    test1 <- function(x, z, sx = 0.3, sz = 0.4) {
      (pi^sx * sz) * (1.2 * exp(-(x - 0.5)^2/sx^2 - (z - 0.5)^2/sz^2)) 
